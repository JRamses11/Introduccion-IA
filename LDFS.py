...
def LDFS(F, nivel, limite, ax):
    estados_visitados = []  # Lista para almacenar todos los estados visitados

    def LDFS_recursivo(F, nivel, limite):
        if not F:
            ...
            EA = F.pop(0)
            print(EA)
            estados_visitados.append(EA)  # Guardar el estado visitado
            ...
            if goaltest(EA):
                ...
                if nivel < limite - 1:
                    ...
                    print("Se llegó al límite, no hay solución")

            dibujar_tablero(EA, ax)  # Dibujar el estado actual del tablero
            plt.pause(0.001)  # Pausa para visualizar el estado antes de avanzar (ajusta el valor para cambiar la velocidad)

            return LDFS_recursivo(F, nivel, limite)  # Llamar recursivamente a la función con la nueva lista F y nivel

    return LDFS_recursivo(F, nivel, limite)
...
limite=1000
fig, ax = plt.subplots()
LDFS(estado_inicial, 0, limite, ax)
plt.show()